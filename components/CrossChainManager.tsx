...
import { ArrowRight, Check, Clock, X, AlertCircle, Loader2, ChevronRight, Activity, Zap, ShieldCheck, Play, Pause, Repeat, ExternalLink, Settings, MoreHorizontal, XCircle } from 'lucide-react';
...
export const CrossChainManager: React.FC = () => {
  const [selectedOp, setSelectedOp] = useState<Operation>(mockOperations[0]);
      ...
      <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex gap-2 bg-neutral-900 p-1 rounded-lg border border-white/10">
               <button 
                  ...
                  className={`px-4 py-2 text-sm font-bold rounded-md transition-all ${activeTab === 'active' ? 'bg-white/10 text-white' : 'text-neutral-500 hover:text-white'}`}
               >
                  Active
               </button>
               <button 
                  onClick={() => setActiveTab('history')}
                  className={`px-4 py-2 text-sm font-bold rounded-md transition-all ${activeTab === 'history' ? 'bg-white/10 text-white' : 'text-neutral-500 hover:text-white'}`}
               >
                  History
               </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <ScrollReveal delay={100} className="lg:col-span-1 h-full">
            <div className="bg-neutral-900/30 border border-white/10 rounded-2xl overflow-hidden h-full flex flex-col">
               <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center">
                  <span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">Recent Activity</span>
                   ...
                   <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div className="flex items-center gap-6 w-full md:w-auto">
                         ...
                         <div className="flex-1 md:w-32 flex flex-col items-center gap-1">
                            ...
                            <span className="text-[10px] uppercase tracking-wider text-nexus-yellow font-bold">
